window.addEventListener("ucnc_load_components", (e) => {
	...
	window.ucnc_app.component('ipv4', window.IPv4FieldComponent);
});
...
window.IPv4FieldComponent = {
	props: {
		...
		tooltip: { type: String, default: "" },
		nullable: { type: Boolean, default: false },
		littleendian: { type: Boolean, default: true }
	},
	data() {
		return {
	...
	methods: {
		...
		updateOctets() {
			if (!this.$root || !this.$root.app_state) return;

			const raw = this.$root.app_state[this.name];
			const val = (typeof raw === 'number') ? (raw >>> 0) : ((Number(raw) >>> 0) || 0);

			if (this.littleendian) {
				// d.c.b.a  (LSB first)
				this.octets[0] = val & 0xFF;
				this.octets[1] = (val >>> 8) & 0xFF;
				this.octets[2] = (val >>> 16) & 0xFF;
				this.octets[3] = (val >>> 24) & 0xFF;
			} else {
				// a.b.c.d  (MSB first)
		...
		updateInteger() {
			...
			if (this.littleendian) {
				// LSB first: a is least significant byte
				intVal =
					((a & 0xFF)) +
					((b & 0xFF) << 8) +
					((c & 0xFF) << 16) +
				...
				// MSB first: a is most significant byte
				intVal =
					((a & 0xFF) << 24) >>> 0 +
					((b & 0xFF) << 16) +
					((c & 0xFF) << 8) +
